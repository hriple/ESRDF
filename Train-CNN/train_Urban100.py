...
# from loss_resdiff import image_compare_loss
...
def valid(model, epoch, args):
    ...
    processed_images = 0  # 计数器，控制最多处理500张图片
    with torch.no_grad():
    ...
    print(f"valid-----Average PSNR 100张): {psnr_score:.2f}")
    print(f"valid-----Average SSIM 100张): {ssim_score:.4f}")
    print(f"valid-----FID 100张: {fid_score:.4f}")
    print(f"valid-----Lpips 100张: {lpips_score:.4f}")
...
def main():
    ...
    parser.add_argument('--hr_dir', type=str, default='/newHome/S2_HHH/zzzz_Pixel_predictor/data/Urban100/train_40_160/hr_160')
    parser.add_argument('--valid_dir', type=str, default='/newHome/S2_HHH/srdiff_data/Urban100/test/HR')
    # # 新增预训练模型加载参数
    parser.add_argument('--pretrained_model', type=str, 
                        default='/newHome/S2_HHH/zzzz_Pixel_predictor/pre_model/DIV2K_40_160_model_20250515-154159/epoch_5000/model.pth', 
                        help='Path to pretrained model weights for resume training')
    parser.add_argument('--low_size', type=int, default=40)
    parser.add_argument('--high_size', type=int, default=160)
    parser.add_argument('--device', type=str, default='cuda:1')  # 保留字符串类型参数
    ...
    dtype = torch.cuda.FloatTensor if args.device.startswith(args.device) else torch.FloatTensor
    ...
    args.results = os.path.join(args.results, f'Urban100_{args.low_size}_{args.high_size}_model_{timestamp}')
    os.makedirs(args.results, exist_ok=True)
    
    args.model_save = os.path.join(args.model_save, f'Urban100_{args.low_size}_{args.high_size}_model_{timestamp}')
    ...
    # 指标图
    plt.figure(figsize=(10, 10))  # 调整图片大小以适应 2x2 布局
    valid_epochs = [i * args.valid_freq for i in range(len(valid_history['psnr']))]
    # # PSNR 子图
    # plt.subplot(2, 2, 1)
    # plt.plot(valid_epochs, valid_history['psnr'], label='PSNR', color='blue')
    # plt.xlabel('Epoch')
    ...
    plt.subplot(2, 2, 1)
    plt.plot(valid_epochs, valid_history['psnr'], label='PSNR', color='blue')
    max_psnr = max(valid_history['psnr'])
    max_index = valid_history['psnr'].index(max_psnr)
    plt.plot(valid_epochs[max_index], max_psnr, 'ro')  # 标出最大值点
    plt.axhline(y=max_psnr, color='r', linestyle='--')  # 添加虚线
    plt.text(valid_epochs[max_index], max_psnr, f'{max_psnr:.4f}', ha='right', va='bottom')  # 添加数值标注
    plt.xlabel('Epoch')
    plt.ylabel('PSNR Value')
    plt.title('Validation PSNR')
    plt.legend()
    ...
    # SSIM 子图
    plt.subplot(2, 2, 2)
    plt.plot(valid_epochs, valid_history['ssim'], label='SSIM', color='green')
    max_ssim = max(valid_history['ssim'])
    max_index = valid_history['ssim'].index(max_ssim)
    plt.plot(valid_epochs[max_index], max_ssim, 'go')  # 标出最大值点
    plt.axhline(y=max_ssim, color='g', linestyle='--')  # 添加虚线
    plt.text(valid_epochs[max_index], max_ssim, f'{max_ssim:.4f}', ha='right', va='bottom')  # 添加数值标注
    plt.xlabel('Epoch')
    ...
    # # FID 子图
    # plt.subplot(2, 2, 3)
    # plt.plot(valid_epochs, valid_history['fid'], label='FID', color='red')
    # plt.xlabel('Epoch')
    ...
    plt.subplot(2, 2, 3)
    plt.plot(valid_epochs, valid_history['fid'], label='FID', color='red')
    min_fid = min(valid_history['fid'])
    min_index = valid_history['fid'].index(min_fid)
    plt.plot(valid_epochs[min_index], min_fid, 'go')  # 标出最小值点
    plt.axhline(y=min_fid, color='g', linestyle='--')  # 添加虚线
    plt.text(valid_epochs[min_index], min_fid, f'{min_fid:.4f}', ha='right', va='top')  # 添加数值标注
    plt.xlabel('Epoch')
    plt.ylabel('FID Value')
    plt.title('Validation FID')
    plt.legend()
	...
	# 在保存完曲线图后，添加以下代码保存超参数信息到txt文件
    hyperparameter_info = f"hr_dir: {args.hr_dir}\n" \
                          f"valid_dir: {args.valid_dir}\n" \
                          f"low_size: {args.low_size}\n" \
                          f"high_size: {args.high_size}\n" \
                          f"device: {args.device}\n" \
                          f"batch_size: {args.batch_size}\n" \
                          f"test_batch_size: {args.test_batch_size}\n" \
                          f"learning_rate: {args.learning_rate}\n" \
                          f"epochs: {args.epochs}\n" \
                          f"model_save: {args.model_save}\n" \
                          f"results: {args.results}\n" \
                          f"valid_freq: {args.valid_freq}\n"

    hyperparameter_file_path = os.path.join(args.results, "hyperparameters.txt")
    with open(hyperparameter_file_path, "w") as file:
        file.write(hyperparameter_info)
