...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/sr_ddpm_20_160_CelebA.json',
    ...
    if opt['phase'] == 'train':
        ...
        for _, val_data in enumerate(val_loader):
            ...
            for i in range(visuals['HR'].shape[0]):
                 ...
                 #  生成保存链接
                 hr_save_path = os.path.join(result_path, 'HR', f'{current_step}_{val_data["Index"][i]}_hr.png')
                 inf_save_path = os.path.join(result_path, 'INF', f'{current_step}_{val_data["Index"][i]}_inf.png')
                 lr_save_path = os.path.join(result_path, 'LR', f'{current_step}_{val_data["Index"][i]}_lr.png')
                 sr_save_path = os.path.join(result_path, 'SR', f'{current_step}_{val_data["Index"][i]}_sr.png')
                 add_save_path = os.path.join(result_path, 'ADD', f'{current_step}_{val_data["Index"][i]}_add_img.png')
                #  保存图片
                 Metrics.save_img(hr_img, hr_save_path)
                 Metrics.save_img(inf_img, inf_save_path)
                 Metrics.save_img(lr_img, lr_save_path)
