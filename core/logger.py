...
def parse(args):
    ...
    if 'train' not in args.phase and args.phase == 'val':
    ...
    # debug，设置日志目录，默认参数修改，但是目前 还没使用过这个
...
# dict_to_nonedict 函数的作用是将一个普通的字典（或其他嵌套的数据结构）转换为一个 NoneDict。
...
# dict2str 函数的作用是将一个字典（dict）转换成一个格式化的字符串，这个字符串可以用于日志记录。
...
# setup_logger 函数用于设置和配置一个日志记录器（logger），以便在Python程序中记录日志信息。
