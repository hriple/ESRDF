import sys
sys.path.append('/newHome/S2_HHH/zzzz')
import argparse
...
# 测试函数
def test(hr_img, model, device, upscale_factor):
    # 加载图像
    image = hr_img
    # print('image',image)
    # 128 * 128
    original_size = image.size
    downsampled_size = (original_size[0] // upscale_factor, original_size[1] // upscale_factor)
    
    # 转换图像为Tensor
    transform = transforms.Compose([
        transforms.Resize(downsampled_size, interpolation=InterpolationMode.BICUBIC), 
        transforms.ToTensor(),
    ...
    with torch.no_grad():
        ...
        output = output.permute(1, 2, 0)  # 转换为HxWxC格式
        mean = torch.tensor([0.5, 0.5, 0.5]).to(device)
        std = torch.tensor([0.5, 0.5, 0.5]).to(device)
        output = (output * std) + mean
        output = output.clamp(0, 1)
        
        output = output.permute(2, 0, 1)  # 转换为HxWxC格式
        output_image = transforms.ToPILImage()(output.cpu())
        return output_image
        
def up_pre_cnn(hr_img, upscale_factor):
    # 模型参数
    upscale_factor = upscale_factor
    model_path = '/newHome/S2_HHH/zzzz_Pixel_predictor/pre_model/CelebA_20_160_model_20250622-234030/epoch_25/model.pth'  # 训练好的模型路径

    # 设备
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

    # 加载模型
    model = PixelPredictor(upscale_factor=upscale_factor).to(device)
    
    model.load_state_dict(torch.load(model_path, map_location=device))

    # 测试模型
    sr_img = test(hr_img, model, device, upscale_factor)
    return sr_img
...
def resize_multiple(img, sizes=(16, 128), resample=Image.BICUBIC, lmdb_save=False):
    ...
    # print('int(sizes[1]/sizes[0])', int(sizes[1]/sizes[0]))
    sr_img = up_pre_cnn(hr_img, int(sizes[1]/sizes[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', '-p', type=str,
                        default='/newHome/S2_HHH/zzzz_Pixel_predictor/data/CelebA/train_20_160/hr_160')
    parser.add_argument('--out', '-o', type=str,
                        default='/newHome/S2_HHH/gitte/ddpm_our/dataset/CelebA/train')

    parser.add_argument('--size', type=str, default='20,160')
